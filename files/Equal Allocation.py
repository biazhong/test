...
def custom_procedure(alternatives, configs, replication):
    ...
    for alt in alternatives:
        alt.set_seed([seed[0] + replication * k + alt.get_args()[0], seed[1], seed[2]])
    ...
    results.append({
        "best alternative args": best_alternative.get_args(),
        "simulation time": simulation_time,
        "wall-clock time": time.time() - start_time,
        "cumulative sample size": cumulative_sample_size,
